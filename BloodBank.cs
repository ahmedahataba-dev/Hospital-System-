...
namespace Hospital_System
{
        ...
        public string GetDashboardInfo()
        {
            int totalBags = 0;
            foreach (var t in _transfers) totalBags += t.Bags;
            string info = $"Total Donors     : {_donors.Count}\n";
            info       += $"Total Transfers  : {_transfers.Count}\n";
            info       += $"Total Bags Moved : {totalBags}\n\n";
            info       += "--- Inventory ---\n";
            foreach (var s in _bloodStocks)
                info += $"  {s.Key,-5}: {s.Value} bags  [{(s.Value < 5 ? "!! CRITICAL" : "OK")}]\n";
            return info;
        }
