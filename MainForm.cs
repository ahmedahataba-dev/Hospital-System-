...
namespace Hospital_System
{
    public class MainForm : Form
    {
        ...
        Panel PagePatients()
        {
            ...
            ly += gap + 4;
            var inpPanel = new Panel { Location = new Point(lx, ly), Size = new Size(1050, 38),
                                       BackColor = Color.FromArgb(232, 245, 255), Visible = false };
        ...
        Panel PageBloodBank()
        {
            ...
            {
                Location = new Point(14, 34), Size = new Size(532, 274),
                ReadOnly = true, BorderStyle = BorderStyle.None,
                BackColor = Color.White, Font = FNorm
            };
        ...
        Panel PageOperations()
        {
            ...
            schedBtn.Click += (s2, e2) =>
            {
                ...
                {
                    PatientName = pat, DoctorName = tbDoc.Text.Trim(),
                    OperationType = tbSType.Text.Trim(),
                    SurgeryDate = dt, Time = tm,
                    RoomNumber = rn.ToString(), IsEmergency = false
                });
                _engine.SaveData();
                RefreshOps();
            ...
            var emergBtn = new Button
            {
                Text = "🚨 EMERGENCY NOW", Size = new Size(330, 44), Location = new Point(14, 84),
                FlatStyle = FlatStyle.Flat, BackColor = AccentRed, ForeColor = Color.White,
            ...
            emergBtn.Click += (s2, e2) =>
            {
                ...
                HospitalEngine.operationsList.Add(new Operation
                {
                    PatientName = pat, DoctorName = "On-Call Surgeon",
                    OperationType = "Urgent Surgery",
                    SurgeryDate = DateTime.Now, Time = DateTime.Now,
                    RoomNumber = roomStr, IsEmergency = true
                });
                _engine.SaveData();
                RefreshOps();
        ...
        Panel PageRoomCleaning()
        {
            ...
            {
                Location = new Point(12, 42), Size = new Size(238, 124),
                Font = FNorm, BorderStyle = BorderStyle.None,
        ...
        Panel PageReceptionBooking()
        {
            ...
            {
                int idx = i;
                tabBtns[i] = new Button { Text = tabNames[i], Font = FBold, Size = new Size(180, 36), Location = new Point(4 + i * 184, 1),
                    FlatStyle = FlatStyle.Flat, BackColor = Color.FromArgb(200, 210, 230), ForeColor = TextDark };
                tabBtns[i].FlatAppearance.BorderSize = 0;
                tabBtns[i].Click += (s, e) => ShowTab(idx);
                tabStrip.Controls.Add(tabBtns[i]);
            }
            ...
            btnBk.Click += (s, e) =>
            {
                ...
                if (cbClin.SelectedIndex < 0) { bookMsg.Text = "Select clinic."; bookMsg.ForeColor = AccentRed; return; }
